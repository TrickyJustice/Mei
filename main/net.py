...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, y, y_t, mask = None, **kwargs):
        B, N, C = x.shape
        ...
        x = x + self.cross_attn(x, y_t, mask)
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, input_size=32, patch_size=2, in_channels=4, hidden_size=1152, output_dim = 512, depth=28, num_heads=16, mlp_ratio=4.0, drop_path: float = 0., window_size=0, window_block_indexes=None, use_rel_pos=False, caption_channels=512, lewei_scale=1.0, config=None, model_max_length=77, **kwargs):
        ...
        self.y_text_embedder = CaptionEmbedder(in_channels=caption_channels, hidden_size=hidden_size, act_layer=approx_gelu, token_nums = model_max_length)
    ...
    def forward(self, x, y, y_t, mask = None, **kwargs):
        ...
        y: (N, 1, 1, C) tensor of caption passed through clip projection layer
        y_t : (N, 1, 77, c) tensor of text embedding of caption obtained through clip text encoder
        """
        B, C, H, W = x.shape
        x = x.to(self.dtype)
        y = y.to(self.dtype)
        y_t = y_t.to(self.dtype)

        pos_embed = self.pos_embed.to(self.dtype)
        self.h, self.w = x.shape[-2]//self.patch_size, x.shape[-1]//self.patch_size
        x = self.x_embedder(x) + pos_embed  # (N, T, D), where T = H * W / patch_size ** 2
        y = self.y_embedder(y, self.training)  # (N, 1, 1, D)
        y_t = self.y_text_embedder(y_t, self.training)
        ...
        if mask is not None:
            if mask.shape[0] != y_t.shape[0]:
                mask = mask.repeat(y_t.shape[0] // mask.shape[0], 1)
            mask = mask.squeeze(1).squeeze(1)
            y_t = y_t.squeeze(1).masked_select(mask.unsqueeze(-1) != 0).view(1, -1, x.shape[-1])
            y_lens = mask.sum(dim=1).tolist()
        else:
            y_lens = [y_t.shape[2]] * y_t.shape[0]
            y_t = y_t.squeeze(1).view(1, -1, x.shape[-1])
            
        for block in self.blocks:
            x = block(x, y_addn, y_t, y_lens)  # (N, T, D) 
    ...
    def initialize_weights(self):
        ...
        nn.init.normal_(self.y_text_embedder.y_proj.fc1.weight, std=0.02)
        nn.init.normal_(self.y_text_embedder.y_proj.fc2.weight, std=0.02)
...
class Mei(nn.Module):
    ...
    def forward(self, x, y, y_t, mask):
        mu, logvar = self.encoder(x, y, y_t, mask)
        std = torch.exp(0.5 * logvar)
        z = mu + std*torch.randn_like(std)
        x = self.decoder(z, y, y_t, mask)
        return x, mu, std, z
...
if __name__ == '__main__':
    ...
    model = Mei(input_size = 32, patch_size = 2, in_channels = 4, hidden_size = 768, depth = 16, output_dim=512, num_heads = 16).to(device)
    latent_image_batch = torch.rand(64, 4, 32, 32).to(device)
    texts = [
        "Quick fox", "Lazy dog", "Bright sun", "Blue sky", "Green grass", "Red rose", "Happy child", "Sad clown",
        "Calm sea", "Loud noise", "Tall tree", "Short man", "Big house", "Small car", "Fast runner", "Slow turtle",
        "Cold breeze", "Warm coffee", "Dark night", "Light day", "Clear water", "Muddy road", "Soft pillow", "Hard rock",
        "Fresh air", "Stale bread", "Quiet room", "Noisy street", "Sharp knife", "Blunt pencil", "Hot tea", "Cool drink",
        "Brave soldier", "Frightened cat", "Heavy rain", "Light drizzle", "Quick response", "Late arrival", "Strong wind", "Weak signal",
        "Bright idea", "Dull moment", "Friendly neighbor", "Angry customer", "Sweet candy", "Sour lemon", "Funny joke", "Serious issue",
        "Clean shirt", "Dirty hands", "Empty bottle", "Full glass", "New phone", "Old book", "Wide smile", "Narrow path",
        "Good news", "Bad weather", "Rich flavor", "Poor decision", "Lively music", "Silent night", "Long journey", "Short break"
    ]


    clipEncoder = CLIPEncoder("cuda")
    y_op, mask = clipEncoder.get_text_embeddings(texts)

    y = y_op.text_embeds
    y_t = y_op.last_hidden_state

    y = y.unsqueeze(1).unsqueeze(2).to(device)
    y_t = y_t.unsqueeze(1).to(device)

    x, mu, std, z = model(latent_image_batch, y, y_t, mask)
    print(x.shape, mu.shape, std.shape, z.shape)
    
    # from thop import profile 
    # flops, params = profile(model, inputs=(latent_image_batch, y, y_t, mask))
